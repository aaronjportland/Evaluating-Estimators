...
* 
* Purpose: Run psmatch2 under several matching specifications, estimate 
* the effect for each, and select the spec with the best post-match 
* balance (lowest mean absolute SMD) as the headline result. 
* 
...
*         Matching/Output/output_code_09_psm.txt (log) 
* 
* Author: Aaron Joseph 
*============================================================================= 
clear all 
...
cap mkdir "$out_match" 
cap log close 
log using "$out_match/output_code_09_psm.txt", text replace 
...
* Select the WINNING spec: lowest mean_abs_smd among valid specs. Ties 
* broken by spec_num (earliest wins). Reuses the tempfile saved above. 
use "`psm_results_raw'", clear 
...
* Post-match balance table + common support graph for the WINNING spec. 
...
* Importance ranking of covariates actually used to compute the pscore. 
...
upsert_effect_row, csvpath(`"$csv_path"') model(`"`this_label'"') method(`"psmatch2"') /// 
	coefname(`"att"') beta(`best_att') se(`best_se') pval(`best_p') n1(`best_nm') n2(`best_nd') /// 
