...
* 
* Purpose: Use adaptive LASSO to select covariates as predictors of $D, 
* then use the selected variables as CEM coarsening dimensions and 
...
*         Matching/Output/output_code_12_lasso_cem.txt (log) 
* 
* Author: Aaron Joseph 
*============================================================================= 
clear all 
...
cap mkdir "$out_match" 
cap log close 
log using "$out_match/output_code_12_lasso_cem.txt", text replace 
 
global Y_main business_practices_sum_2019 
global D treatment_assignment_binary 
 
cap confirm file "$out_match/Matching_output_code_03_sample.dta" 
if _rc { 
	di as error "ERROR: sample file not found." 
	cap log close 
	exit 601 
} 
...
if "`Xlasso'" == "" { 
	di as error "ERROR: no variables survived adaptive lasso selection." 
	cap log close 
	exit 198 
} 
...
* Importance ranking of LASSO-selected covariates. 
