...
* 
* Purpose: Run AIPW and IPWRA, each with logit and probit propensity 
* models, and select the best spec by post-weighting balance (mean 
* |SMD|) -- same logic as code 10's psmatch2 spec selection. 
* 
* respondent_owner_2018 is dropped from $Xdr -- collinear on this 
...
* index : label : estimator : propensity link 
...
* Select the winning spec: lowest mean|SMD| among valid specs. 
...
* Overlap graphic for the winning spec's propensity model. 
use "$out_match/Matching_output_code_03_sample.dta", clear 
...
upsert_effect_row, csvpath(`"$csv_path"') model(`"`this_label'"') method(`"`best_spec_label'"') /// 
	coefname(`"atet"') beta(`best_beta') se(`best_se') pval(`best_p') n1(`best_n1') n2(`best_n2') /// 
