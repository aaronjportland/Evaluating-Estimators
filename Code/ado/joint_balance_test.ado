...
program define joint_balance_test, rclass 
    ...
    *-------------------------------------------------------------------- 
    * F-test: regress D on covariates, test joint significance. 
    * H0: covariates jointly do NOT predict treatment assignment, i.e. 
    * balance holds jointly, not just variable-by-variable. 
    * Supports weights natively via regress. 
    *-------------------------------------------------------------------- 
    if "`wtvar'" == "" { 
    ...
    *-------------------------------------------------------------------- 
    * mvtest means: multivariate (Hotelling's T2-type) test of whether 
    * treated/control groups have equal covariate mean VECTORS. 
    * NOTE: mvtest does not support weights. This is run UNWEIGHTED on 
    * the `touse' subsample, so for CEM (where weights mainly correct 
    * within-stratum counts) this is a reasonable approximation, but for 
    * PSM/entropy balancing (continuous weights) it is a weaker, 
    * approximate check -- not equivalent to the weighted balance table. 
    *-------------------------------------------------------------------- 
    cap which mvtest 
    if _rc { 
